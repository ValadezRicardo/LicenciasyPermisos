...
function create($prefijo){
    ...
    // execQueryP("update permisoprefijo set  deleted=1");
    $q=execQueryP("INSERT INTO permisoprefijo(descripcion) 
    VALUES('$desc')");
    // $q=execQueryP("UPDATE permisorangos set active = 0");
    echo '<script>window.location=window.location.origin+window.location.pathname</script>';
}

function delete($prefijo){
    $id=$prefijo['id'];
    $q=execQueryP("UPDATE permisoprefijo SET deleted=1 WHERE id=$id");
    $q=execQueryP("UPDATE permisorangos set active = 0");
}
...
<!--      <h2>PLATAFORMA PERMISOS</h2>
      <h3>USUARIO QUE PUEDE VER ESTA SECCIÓN: SUPERADMIN</h3>-->
        <h3>NUEVO PREFIJO</h3>
        <form class="my-5" action="permisoprefijo.php?a=c"  method="POST" style="margin-top:0px!important">
            <div class="row">
    ...
    <section class="top-element" style="margin-top:5px!important">                
        <table id="FoliosLicencias" class="display table table-striped" style="width:100%">
            ...
            <tbody>
            <?php
                 $q=execQueryP("SELECT * FROM permisoprefijo order by id desc");
    ...
    <form action="permisoprefijo.php?a=d" style="display:none;"  method="POST"> 
