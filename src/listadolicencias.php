...
<!--      <h2>PLATAFORMA LICENCIAS</h2>
      ...
      <a type="button" class="btn btn-secondary" href="registrolicencias.php">Registrar Folio</a>
     
    </section>
    <hr class="my-5">
    <section>
      <h3 class="mt-2 mb-5"><strong>Selecciona el rango de fechas del listado:</strong> </h3>
    <?php
        $date =date("Y-m-d");
    ...
    ?>
      <form name="form" action="listadolicencias.php"  method="POST">
        <div class="row">
    ...
    <section class="mb-5">
        <table id="FoliosLicencias" class="display table table-striped" style="width:100%">
            ...
            <tbody>
                <?php
                 $q=execQuery("select l.*,lp.descripcion as prefijo,lp.id as prefijo_id,ul.nombre usuarioNombre from licencia l left join licenciaprefijo lp on l.id_prefijo=lp.id join usuario ul on l.usuario_id=ul.id where usuario_id=$id or 1=$rol_id and expedicion>='$fecha1' and expedicion<='$fecha2' order by expedicion");
    ...
    <form action="registrolicencias.php?a=d" style="display:none;"  method="POST"> 
      ...
      function generateQr(folio,prefijo_id){
        jQuery('#qrcodeTable').text("");
           jQuery('#qrcodeTable').qrcode({
            render	: "canvas",
            text	: "<?php echo "http://$_SERVER[HTTP_HOST]/src/consultalicenciaciudadano.php?folio=";?>"+folio+"&prefix="+prefijo_id
          });	
       }
...
getDataUri("/imgs/L1-01.jpg", function(dataUri) {  
    ...
    doc.text(tipoLicencia,140, 64, 'center')
...
getDataUri("/imgs/L1-02.jpg", function(dataUri) { 
    ...
    // Return the data url via callback
    foto=dataURL;
    doc.addImage(dataURL,'JPEG',175,185,20,30);
    // Mark the canvas to be ready for garbage 
    // collection
    canvas = null;
  };
...
var dateTime2 = moment(date1).add(30,"days").format(format2);
